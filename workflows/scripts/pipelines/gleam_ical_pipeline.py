...
from wrappers.arlexecute.pipelines.pipeline_arlexecute import ical_arlexecute
...
if __name__ == '__main__':
    ...
    ical_list = ical_arlexecute(future_vislist, model_imagelist=model_list, context='wstack', calibration_context='TG',
                                do_selfcal=True, controls=controls, scales=[0, 3, 10], algorithm='mmclean', nmoment=3,
                                niter=1000, fractional_threshold=0.1, threshold=0.1, nmajor=5, gain=0.25,
                                deconvolve_facets=8, deconvolve_overlap=32, deconvolve_taper='tukey', vis_slices=ntimes,
                                timeslice='auto', global_solution=False, psf_support=64)
    
    # In[ ]:
    
    log.info('About to run ical_arlexecute')
