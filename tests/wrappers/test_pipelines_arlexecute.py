...
from wrappers.arlexecute.pipelines.pipeline_arlexecute import ical_arlexecute, continuum_imaging_arlexecute
...
class TestPipelineGraphs(unittest.TestCase):
    ...
    def test_ical_pipeline(self):
        ...
        ical_list = \
            ical_arlexecute(self.vis_list, model_imagelist=self.model_imagelist, context='2d', calibration_context='T',
                            do_selfcal=True, controls=controls, global_solution=False, algorithm='mmclean', facets=1,
                            scales=[0, 3, 10], niter=1000, fractional_threshold=0.1, nmoments=2, nchan=self.freqwin,
                            threshold=2.0, nmajor=5, gain=0.1, deconvolve_facets=8, deconvolve_overlap=16,
                            deconvolve_taper='tukey')
